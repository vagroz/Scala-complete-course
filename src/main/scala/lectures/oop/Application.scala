...
class Application(isTestEnv: Boolean) {
  ...
  val usefulService: UsefulService = if (isTestEnv)
   ...
   new SQLAPI("production db Resource") with ProductionServiceImpl //передайте "production Resource" в качестве ресурсв в конструктор SQLAPI
